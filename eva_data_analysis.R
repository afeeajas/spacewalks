...
graph_file = 'comulative_eva_graph.png'
...
#data.pop(0)
## Comment out this bit if you don'duration_dt want the spreadsheet
...
date <- df$date
time <- df$time
cummulative_time = duration_dt[2:length(duration_dt)]

p <- ggplot(df, aes(x = date, y = cummulative_time)) +  
  geom_point() +
  geom_line() +  
